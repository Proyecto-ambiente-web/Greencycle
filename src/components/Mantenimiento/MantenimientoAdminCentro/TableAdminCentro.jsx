...
function TableMaterialHead(props) {
  const {
    order,
    orderBy,
    numSelected,
    ...
    <TableHead>
      <TableRow>
        <TableCell>
          ...
          <Tooltip title='Crear'>
            <IconButton component={Link } to='/user/CreateAdminCentro'> {/* Crear material*/}
              <AddIcon/>
            </IconButton>
          </Tooltip>
        </TableCell>
...
function TableMaterialToolbar(props) {
  ...
  const update = () => {
    return navigate(`/UpdateAdminCentro/${idSelected}`)    //Ruta para actualizar material
  }
  return (
    <Toolbar
      sx={{
    ...
    >
      {numSelected > 0 ? (
        ...
        <Typography
          ...
          component='div'
        >
          Mantenimiento administradores de centro
        </Typography>
      )}
...
export default function TableAdminCentro() {
  ...
  useEffect(() => {
    UsuarioService.usuarioAdminCentro(2)
      .then((response) => {
